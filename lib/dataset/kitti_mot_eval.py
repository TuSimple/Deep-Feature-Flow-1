...
# --------------------------------------------------------
"""
given a kitti mot imdb, compute mAP
"""
...
def parse_kitti_mot_rec(annotation_file_path, classhash, img_ids, defaultIOUthr=0.5, pixelTolerance=10):
    """
    ...
    for obj in annotation_list:
        label = obj.strip().split(' ')

        obj_dict = dict()
        obj_dict['label'] = classhash[label[2]]
        obj_dict['bbox'] = [float(label[6]),
                            float(label[7]),
                            float(label[8]),
                            float(label[9])]
...
def kitti_mot_eval(detpath, imageset_file, classnames, annocache, ovthresh=0.5):
    """
    ...
    classhash = dict(zip(classnames, range(0,len(classnames))))
    ...
    if not os.path.isfile(annocache):
        ...
        for ind, image_filename in enumerate(img_basenames):
            annotation_file_path = image_filename.replace("image_02", 'label_new')+'.txt'
            assert os.path.exists(annotation_file_path), "Path does not exist:{}".format(annotation_file_path)

            recs.append(parse_kitti_mot_rec(annotation_file_path, classhash, gt_img_ids[ind]))
            if ind % 100 == 0:
    ...
    ap = np.zeros(len(classnames))
    for c in range(1, len(classnames)):
